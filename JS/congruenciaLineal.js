...
document.getElementById('generarDatosBtn').addEventListener('click', function() {
    ...
    `<table id="t02" style="width: 260px; ">
        <th>Formula</th>
        <th>Valor</th>
    </table>`;
...
function filaParametros(a,c,n,m){
    return `
    <tr style="background-color: white;"><td>M = 2<sup>g</sup></td>  <td>${m}</td></tr>
    <tr style="background-color: white;"><td>a = 1 + 4*k</td>  <td>${a}</td></tr> 
    <tr style="background-color: white;"><td>C = N. primo cercano a M</td>  <td>${c}</td></tr>
    <tr style="background-color: white;"><td>N = M = 2<sup>g</sup></td>  <td>${n}</td></tr>
    `;
}
